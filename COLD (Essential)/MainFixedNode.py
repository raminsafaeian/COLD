...
AlgorithmName = {}
AlgorithmName[Algorithm.COLD_Essential]   = 'COLD_Essential'
...
Alg = [Algorithm.COLD_Essential,Algorithm.PCAlg]
...
for p in nNodes:
    ...
    for e in range(int(.1*p*(p-1)/2),int(.51*p*(p-1)/2),int(.1*p*(p-1)/2)):
        nAvgCnter = 0
        for AlgIdx in range(len(Alg)):
        ...
        while(nAvgCnter<nAvg):
           ...
           ##############################################################################
            ...
            ###
            ##############################################################################
            print("COLD_Essential algorithm Running...")
            ...
            AvgTime[Alg.index(Algorithm.COLD_Essential)][p][e]= (end1-start1) 
